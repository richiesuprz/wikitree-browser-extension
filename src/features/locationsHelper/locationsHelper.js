          ...
          $(fieldSelectors).on("focus", async function () {
            ...
            if (!window.locationsHelperInitDone) {
              ...
              if (window.bdLocations) {
                console.log("[locHelper] Family birth/death locations available:", window.bdLocations);
              }
            }
          });
        })
  ...
  if (window.bdLocations) {
    ...
    for (const aLoc of window.bdLocations) {
      ...
      if (sim > 0.8 || sugNorm.includes(famNorm) || famNorm.includes(sugNorm)) {
        console.log(`[locHelper] Match check:
  Input:  '${sugNorm}'
  Family: '${famNorm}'
  Sim:    ${sim.toFixed(4)}
...
function fixText(added_node, userInput, dText, innerBit, innerBitText) {
  ...
  cleanText = stripAliasAnnotations(cleanText); // drop aka/bracketed annotations
...
// Remove trailing aliases or bracketed annotations from suggestion text/data-val.
// Examples removed:
//   "... aka Bromborough [Mersey, UK-ENG]"
//   "... [Mersey, UK-ENG]"
function stripAliasAnnotations(raw) {
  if (!raw) return "";
  let s = raw;
  // Drop anything after "aka" (case-insensitive) including the aka token
  s = s.replace(/\baka\b.*$/i, "");
  // Drop a single trailing bracketed segment like "[Mersey, UK-ENG]"
  s = s.replace(/\s*\[[^\]]*\]\s*$/, "");
  // Trim trailing standalone codes like "[XX-YY]" even if no bracket remains
  s = s.replace(/\s*[([][^\]\)]*[\]\)]\s*$/, "");
  s = s
    .replace(/\s{2,}/g, " ")
    .trim()
...
async function locationsHelper() {
  ...
  async function prepareAndInject(container, locationField, state, term, mutations) {
    ...
    try {
      ...
      applySuggestionCorrections(container, locationField, userInput, mutations);
    } finally {
  ...
  function attachObserverToSuggestions() {
    ...
    if (added > 0) console.log(`attachObserverToSuggestions: ${cnt} containers found, attached observers to ${added}`);
  }
  ...
  setTimeout(function () {
    ...
    $(document)
      ...
      .on("mousedown.locationsHelper", ".autocomplete-suggestion", function () {
        ...
        const clean = stripAliasAnnotations(stripLocationDates(val || fromText));
        if (clean !== val) {
          $(this).attr("data-val", clean);
          dbg("mousedown sanitize data-val", { before: val, fromText, after: clean });
        }
        ...
        if (activeEl && /Location|photo_location|Email/.test(activeEl.id || activeEl.name || "")) {
          queueMicrotask(() => {
            ...
            const after = stripAliasAnnotations(stripLocationDates(before));
            if (after !== before) {
...
function applySuggestionCorrections(container, locationField, userInput, mutations_list) {
  ...
  mutations_list.forEach((mutation) => {
    mutation.addedNodes.forEach((node) => {
      if (node.nodeType === 1 && node.classList?.contains("autocomplete-suggestion")) {
        suggestionsToProcess.push(node);
      }
    });
  });

  // Also include augmented suggestions that were just injected but haven't been processed yet
  container.querySelectorAll(".autocomplete-suggestion:not([data-loc-helper-processed])").forEach((node) => {
    if (!suggestionsToProcess.includes(node)) {
      suggestionsToProcess.push(node);
    }
  });

  suggestionsToProcess.forEach(async function (added_node) {
    try {
      dbg("Processing suggestion node", {
      ...
      if (!inputRange) {
        goodDate = true; // no usable input date
        ...
        if (!yearRangeMatch) {
          goodDate = true; // no suggestion range
          ...
          // Check overlap
          if (rangeStart && inputRange.end < rangeStart) goodDate = false;
          if (rangeEnd && inputRange.start > rangeEnd) goodDate = false;
          dbg("date window eval", { rangeStart, rangeEnd, inputRange, goodDate });
        }
      }
      ...
      if ($inner.length) {
        highlightSearchWords(userInput, dText, $inner);
      }
...
function addNewSuggestion(added_node, userInput, term, record, villages = []) {
  ...
  for (let i = 0; i < villages.length + 1; i++) {
    ...
    const newSuggestion = formSuggestionElement(item, userInput);
...
fixText = function (added_node, userInput, dText, innerBit, innerBitText) {
  ...
  try {
    if (
      ...
      const cleanVal = stripAliasAnnotations(stripLocationDates((dText || "").trim()));
